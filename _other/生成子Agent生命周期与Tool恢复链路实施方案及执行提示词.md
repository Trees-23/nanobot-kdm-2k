...
直接在 `_other` 根目录生成以下两个文件，不要创建子目录或版本目录：

```text
_other/子Agent生命周期与Tool恢复链路实施方案.md
_other/执行子Agent生命周期与Tool恢复链路实施方案提示词.md
```

如果目标文件已存在，必须先阅读并判断它是否属于本任务；不得盲目覆盖用户已有内容。
...
- 阅读代码、测试、文档和 Git 历史；
- 运行不改变仓库状态的检查；
- 直接在 `_other` 根目录创建两个 Markdown 输出文件；
...
Git 交付必须遵循 `AGENTS.md`：不在 `main` 开发，默认在当前仓库目录使用 `git switch`；只暂存上述两个明确文件，中文提交并推送当前任务分支，维护同一个 PR；未经用户明确确认不得合并 `main`。除两个目标文件外，仓库内不得产生缓存、截图、临时报告或其他新文件；临时数据必须放在仓库外并在结束前清理。
...
1. 引用 `_other/子Agent生命周期与Tool恢复链路实施方案.md`，要求实施 AI 先读计划再读实际代码。
...
- 两个文件均直接位于 `_other` 根目录，没有创建子目录或版本目录；
